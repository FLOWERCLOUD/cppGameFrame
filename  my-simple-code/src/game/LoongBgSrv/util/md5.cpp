...
char* MD5String(char* szString)
{
	...
	alg.Update( reinterpret_cast<unsigned char*>(szString), static_cast<unsigned int>(nLen));
	alg.Finalize();
	...
	try
	{
		...
		if ((file = fopen (szFilename, "rb")) != NULL)
		{
			while ( (nLen = fread (chBuffer, 1, 1024, file) ))
				alg.Update(chBuffer, static_cast<uint4>(nLen));
...
void md5::Encode(uchar* dest, uint4* src, uint4 nLength)
{
	...
	{
		dest[j] = static_cast<uchar>((src[i] & 0xff));
		dest[j+1] = static_cast<uchar>(((src[i] >> 8) & 0xff));
		dest[j+2] = static_cast<uchar>(((src[i] >> 16) & 0xff));
		dest[j+3] = static_cast<uchar>(((src[i] >> 24) & 0xff));
	}
}
...
void md5::Decode(uint4* dest, uchar* src, uint4 nLength)
{
	...
	for (i = 0, j = 0; j < nLength; i++, j += 4)
	{
		dest[i] = (static_cast<uint4>(src[j])) | ((static_cast<uint4>(src[j+1]))<<8) |
			      ((static_cast<uint4>(src[j+2]))<<16) | ((static_cast<uint4>(src[j+3]))<<24);
	}
}
