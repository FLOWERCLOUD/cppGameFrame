...
static int luabufferevent_get_input(lua_State* L)
{
	...
	if (!luabufferevent->bufferevent) return 0;
	struct evbuffer* evbuf = bufferevent_get_input(luabufferevent->bufferevent);
	if (!evbuf) return 0;

	int ret = push_evbuffer(L, evbuf);
	return ret;
}
...
static int luabufferevent_get_output(lua_State* L)
{
	...
	if (!luabufferevent->bufferevent) return 0;
	struct evbuffer* evbuf = bufferevent_get_output(luabufferevent->bufferevent);
	if (!evbuf) return 0;
