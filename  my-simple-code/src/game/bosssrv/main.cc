...
#include <game/bosssrv/php/htmlclient.h>
...
int main(int argc, char **argv)
{
	...
	if (!configPath)
	{
		...
		if (!(getcwd(path,
				FILENAME_MAX)))
		{
			printf("BossSrv getcwd error! --- vesion: .%2f\n", BOSSSRVVERSION);
			return -1;
		}
		std::string tmp(path);
	...
	LOG_INFO << "============ Start BossSrv ============ version "  << BOSSSRVVERSION;
	...
	BossSrv server(&loop, listenAddr);
	server.start();
	loop.loop();
	server.stop();
	CHtmlClient::finalise();
	LOG_INFO << "============  Stop BossSrv ============ version "  << BOSSSRVVERSION;
