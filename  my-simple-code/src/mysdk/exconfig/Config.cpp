...
void apply_setting(string & str, ConfigSetting & setting)
{
	...
	setting.AsFloat = static_cast<float>(atof(str.c_str()));

	/* check for verbal yes/no answers */
	if(str.length() > 1)
	{
		// this might be a yes/no?
		if(str.size() >= 3 && !strncasecmp("yes", str.c_str(), 3))
		{
			setting.AsBool = true;
			setting.AsInt = 1;
		}
		else if(str.size() >= 2 && !strncasecmp("no", str.c_str(), 2))
		{
...
bool ConfigFile::SetSource(const char* file, bool ignorecase)
{
	...
	if(file != 0)
	{
			...
			parse:
				...
				if(in_block)
				{
					...
					if(in_multiline_quote)
					{
						...
						{
							LOG_ERROR << "Quote without variable.";
							return false;
						}
...
#ifdef _CONFIG_DEBUG
						LOG_DEBUG << "Block: " << current_block.c_str()
												<< ", Setting: " << current_variable.c_str()
												<< ", Value: " << current_setting_struct.AsString.c_str();
#endif
		...
		{
			LOG_ERROR << "Exception in config parsing.";
			return false;
		}
